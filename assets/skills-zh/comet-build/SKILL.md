...
Test Cases: openspec/changes/<name>/test-cases.md。计划中的每个实现任务必须关联到一个或多个测试/验证用例；测试用例可以是单元、集成、端到端、视觉、手动、构建、lint、可访问性或其他适合该 change 的证据。
```

技能加载后，按其指引制定计划。计划文件和执行反馈必须使用触发本次工作流的用户请求语言。计划要求：
- 保存至 `docs/superpowers/plans/YYYY-MM-DD-<feature>.md`
- 引用设计文档，拆分为可执行任务
- 引用 `openspec/changes/<name>/test-cases.md`，并说明每个任务对应的验证方式；不得要求维护整个项目的测试用例全集
- **Plan 文件头必须包含关联元数据**：
...
Test Cases: openspec/changes/<name>/test-cases.md。按任务执行前先确认对应测试/验证用例，必要时先补充或修正该矩阵；不要把所有验证都强行转成单元测试代码。
```
