...
Test Cases: openspec/changes/<name>/test-cases.md。遵循 `/comet` 的验证矩阵规则；计划中的每个实现任务必须关联到对应验证项。
```
...
若计划任务包含适合自动化测试的功能、bugfix、重构或行为变更，必须使用 Skill 工具加载 Superpowers `test-driven-development` 技能，并按其 RED/GREEN/REFACTOR 执行。`test-cases.md` 只记录验证矩阵和证据：适合 TDD 的用例记录 RED/GREEN/REFACTOR 证据，不适合 TDD 的用例记录不适用原因和替代验证方式。
...
- 按计划执行任务
- 完成 tasks.md 勾选（`- [ ]` → `- [x]`）
- 完成或更新 `test-cases.md` 中与该任务相关的验证方式、通过标准和待收集证据
- 每个任务完成后提交代码
