...
1. **立即执行：** 使用 Skill 工具加载 Superpowers `writing-plans` 技能。禁止跳过此步骤。技能加载后，ARGUMENTS 必须包含：`Language: 使用触发本次工作流的用户请求语言输出`；`CodeGraph Context: $COMET_CODEGRAPH_CONTEXT_FILE。遵循 /comet 的 CodeGraph 代码证据规则`；`Test Cases: openspec/changes/<name>/test-cases.md。遵循 /comet 的验证矩阵规则；每个实现任务必须关联到对应验证项`
...
- 引用设计文档，拆分为可执行任务
- 引用 `openspec/changes/<name>/test-cases.md`，并说明每个任务对应的验证方式
- **Plan 文件头必须包含关联元数据**：
...
计划已写入当前分支。在开始执行前，**一次性询问用户**选择工作区隔离方式和执行方式；`tdd_mode` 与 `review_mode` 已有 full workflow 默认值，只有用户明确要求覆盖默认值时才询问：
...
这是用户决策点。**必须按 `comet/reference/decision-point.md` 的协议暂停并等待用户明确选择隔离方式和执行方式**。full workflow 默认 `tdd_mode: tdd`、`review_mode: thorough`；只有用户明确要求覆盖默认值时，才修改这两个字段。不得根据推荐规则自行选择 `branch` 或 `worktree`，也不得根据推荐规则自行选择执行方式。推荐规则只能用于说明建议，不能替代用户确认。
...
full workflow 默认使用 `tdd`。只有用户想覆盖默认值时才询问。

| 选项 | 含义 | 适用场景 |
|------|------|---------|
| `tdd` | 每个任务先写失败测试再写实现 | full workflow 默认。变更涉及业务逻辑、新功能、API |
| `direct` | 直接实现，不强制 TDD 流程 | 仅当用户明确选择跳过强制 TDD 时使用。hotfix/tweak preset 默认使用 `direct` |

如果用户覆盖默认值，运行 `"$COMET_BASH" "$COMET_STATE" set <name> tdd_mode <tdd|direct>`。

**代码审查模式**：

full workflow 默认使用 `thorough`。只有用户想覆盖默认值时才询问。

| 选项 | 含义 | 适用场景 |
|------|------|---------|
| `off` | 不自动派发代码审查 | 文档、配置、文案、小范围低风险任务 |
| `standard` | 只在任务完成后运行一次最终轻量代码审查；若发现问题，最多自动修复一轮，然后交给用户决策 | 普通低到中风险改动 |
| `thorough` | 按批次或风险边界运行合并审查，最后再运行一次完整审查 | full workflow 默认。高风险、多模块、架构或安全相关改动 |

如果用户覆盖默认值，运行 `"$COMET_BASH" "$COMET_STATE" set <name> review_mode <off|standard|thorough>`。
...
`tdd_mode` 是脚本级硬约束。full workflow 初始化默认写入 `tdd`；离开 build 阶段前它必须仍为 `tdd` 或 `direct`，否则 `comet-guard.sh build --apply` 和 `comet-state transition build-complete` 都会失败。

`review_mode` 是脚本级硬约束。full workflow 初始化默认写入 `thorough`；离开 build 阶段前它必须仍为 `off`、`standard` 或 `thorough`，否则 `comet-guard.sh build --apply` 和 `comet-state transition build-complete` 都会失败。旧状态文件若没有该字段，按兼容路径继续，但恢复时应补写该字段。
...
**验证账本衔接（Superpowers + Comet）**：

`test-cases.md` 只作为验证账本：记录场景、关联任务、验证方式、通过标准、证据位置和实际结果。不要在这里复制 Superpowers `test-driven-development` 或 `verification-before-completion` 的流程；这些流程由对应 Superpowers 技能执行。Comet 只要求把执行产生的关键证据摘要写回账本。

加载 `subagent-driven-development` 或 `executing-plans` 时，ARGUMENTS 必须包含同一 Language 约束、CodeGraph 约束和验证矩阵约束：

```text
Language: 使用触发本次工作流的用户请求语言输出。
CodeGraph Context: $COMET_CODEGRAPH_CONTEXT_FILE。遵循 `/comet` 的 CodeGraph 代码证据规则。
Test Cases: openspec/changes/<name>/test-cases.md。按 `/comet` 的验证矩阵规则确认、补充或修正对应验证项。
Evidence Ledger: 不复制 TDD 或完成前验证流程；只把 Superpowers 技能和项目命令产出的实际证据摘要写回 `test-cases.md`。
```
...
执行期间必须完成 tasks.md 勾选，更新相关 `test-cases.md` 的验证方式、通过标准和证据，并在每个任务完成后提交代码。
...
```text
Language: 使用触发本次工作流的用户请求语言输出。
CodeGraph Context: $COMET_CODEGRAPH_CONTEXT_FILE。遵循 `/comet` 的 CodeGraph 代码证据规则判断新增影响面。
```
...
- tasks.md 全部勾选
- `test-cases.md` 已按本 change 补全，且每个已完成 task 至少关联一个验证用例或明确说明不适用原因
- 对适合自动化测试的功能、bugfix、重构或行为变更，已使用 Skill 工具加载 Superpowers `test-driven-development` 技能，并把该技能产生的关键 RED/GREEN/REFACTOR 证据摘要记录到任务提交说明或 `test-cases.md`
- 代码已提交
