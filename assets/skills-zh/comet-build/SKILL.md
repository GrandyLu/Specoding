...
`test-cases.md` 只记录验证矩阵和证据：适合 TDD 的用例记录 RED/GREEN/REFACTOR 证据，不适合 TDD 的用例记录不适用原因和替代验证方式。
...
TDD Discipline: 适合自动化测试的用例按 Superpowers `test-driven-development` 执行，并把 RED/GREEN/REFACTOR 证据写回 `test-cases.md` 或任务提交说明；不适用 TDD 的用例记录原因和替代验证证据。
```
...
- 对适合自动化测试的功能、bugfix、重构或行为变更，已使用 Skill 工具加载 Superpowers `test-driven-development` 技能，并记录 RED/GREEN/REFACTOR 证据；不适用 TDD 的任务已在 `test-cases.md` 记录原因和替代验证方式
- 代码已提交
